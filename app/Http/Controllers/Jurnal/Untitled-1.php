        ...
        if (User::setIjin('Jurnal Angkatan Ayam')) {
            ...
            if ($request->key == 'pakan') {
                ...
                if (empty(KartuStok::where('hari', $request->hari_pakan)->where('tipekartu', 'pakan')->first()->masuk)) {
                    ...
                    $stokkandang = StokKandang::where('kandang_id', $data->kandang)->where('tipe', '2')->where('produk_id', $request->jenis_pakan)->max('sisa');
                    // return response()->json([$stokkandang]);
                    // return response()->json([$data]);
                    if ($stokkandang >= $request->keluar) {
                        $sisa_total = $stokkandang - $cek_masuk->first()->masuk + $request->masuk;
                        $sisa_tot = (int)($sisa_total);
                    }
                    StokKandang::where('kandang_id', $data->kandang)
                        ->where('tipe', '2')
                        ->where('produk_id', $request->jenis_pakan)->update(['sisa' => $sisa_tot]);
            ...
            if ($request->key == 'ovk') {
                ...
                if (empty(KartuStok::where('tipekartu', 'ovk')->where('hari', $request->hari_ovk)->first())) {
                    ...
                    if ($request->ovk_keluar > 0) {
                        $pakan  =   StokKandang::where('produk_id', $request->jenis_ovk)
                            ->where('kandang_id', $data->kandang)
                            ->get();
                    ...
                    $stokkandang = StokKandang::where('kandang_id', $data->kandang)->where('tipe', '1')->where('produk_id', $request->jenis_ovk)->max('sisa');
                    // return response()->json([$stokkandang]);
                    // return response()->json([$data]);
                    if ($stokkandang >= $request->keluar) {
                        $sisa_total = $stokkandang - $cek_masuk->first()->masuk + $request->masuk;
                    ...
                    StokKandang::where('kandang_id', $data->kandang)
                        ->where('tipe', '1')
                        ->where('produk_id', $request->jenis_ovk)->update(['sisa' => $sisa_tot]);
    ...
    public function excel(Request $request)
    {
        ...
        foreach ($prod_import[0] as $urut => $line) {
            ...
            if ($urut != 0) {
                ...
                if (in_array(date('Y-m-d', strtotime($line[0])), $date)) {
                    if (!empty($line['jenis'])) {
                        $jenis = Produk::where('nama', 'like' . '%' . $line['jenis'] . '%')->first();
                    } else {
                        $jenis = 0;
                    }
                    if (!empty($line['datang'])) {
                        $masuk = $line['datang'];
                    } else {
                        $masuk = 0;
                    }
                    if (!empty($line['kghari'])) {
                        $keluar = $line['kghari'];
                    } else {
                        $keluar = 0;
                    }
                    if (!empty($line['mati'])) {
                        $populasi_mati = $line['mati'];
                    } else {
                        $populasi_mati = 0;
                    }
                    if (!empty($line['afkir'])) {
                        $populasi_afkir = $line['afkir'];
                    } else {
                        $populasi_afkir = 0;
                    }
                    if (!empty($line['panen'])) {
                        $populasi_panen = $line['panen'];
                    } else {
                        $populasi_panen = 0;
                    }
                    if (!empty($line['hari'])) {
                    ...
                    if (!empty($line['keterangan'])) {
                        $keterangan = $line['keterangan'];
                    } else {
                        $keterangan = 0;
                    }

                    $kandang = Setup::where('nama', $line['kandang'])->first();
                    if (!empty($kandang)) {
                        $kandang = $kandang->id;
                    } else {
                        $kandang = 0;
                    }
                    $jenis_pakan = Produk::where('nama', $line['jenis_pakan'])->first();
                    if (!empty($jenis_pakan)) {
                        $jenis_pakan = $jenis_pakan->id;
                    } else {
                        $jenis_pakan = 0;
                    }
                    $ovk = Produk::where('nama', $line['ovk'])->first();
                    if (!empty($ovk)) {
                        $ovk = $ovk->id;
                    } else {
                        $ovk = 0;
                    }
                    $strain = Setup::where('nama', $line['strain'])->first();
                    if (!empty($strain)) {
                        $strain = $strain->id;
                    } else {
                        $strain = 0;
                    ...
                    if (KartuStok::where('hari', $hari)->where('tipekartu', 'pakan')->first()) {
                        // dd('pakan');
                        $kartu = KartuStok::where('hari', $hari)->where('tipekartu', 'pakan')->update([
                            'tanggal_masuk'     => date("Y-m-d", strtotime($line['tgl'])),
                            'hari'    => $hari,
                            'masuk'    => $masuk,
                            'keluar'    => $keluar,
                            'jenis'    => $jenis_pakan,
                            'tipekartu'    => "pakan",
                            'recording_id'    => $line['periode'],
                            'tanggal_kartu'    =>  date("Y-m-d", strtotime($line['tgl'])),
    ...
    public function edit_record(Request $request)
    {
        ...
        if ($request->id_datang == 'id_datang') {
            ...
            $stokan = StokKandang::where('kandang_id', $kandang_id->kandang)
                ->where('tipe', '2')
                ->where('produk_id', $request->jenis)->first()->sisa;
            if (!empty(DB::table('kartustok')->where('hari', $request->hari_datang)->where('tipekartu', 'pakan')->first())) {
                // dd([$stokan, $request->jumlah]);
                $update = DB::table('kartustok')->where('hari', $request->hari_datang)->where('tipekartu', 'pakan')->update(['masuk' => $request->jumlah, 'jenis' => $request->jenis]);

                StokKandang::where('kandang_id', $kandang_id->kandang)
                    ->where('tipe', '2')
                    ->where('produk_id', $request->jenis)->update(['sisa' => ($stokan + $request->jumlah_awal - $request->jumlah)]);
                $data['status']         =   200;
                return $data;
            } else {
        ...
        if ($request->id_keluar == 'id_keluar') {
            ...
            // dd([$kandang, $kandang_id->kandang]);
            $update = DB::table('kartustok')->where('hari', $request->hari_keluar)->where('tipekartu', 'pakan');
            // dd(DB::table('kartustok')->where('hari', $request->hari_keluar)->where('tipekartu', 'pakan')->first());
            ...
            if (!empty(DB::table('kartustok')->where('hari', $request->hari_keluar)->where('tipekartu', 'pakan')->first())) {
                ...
                $update->keluar = $request->keluar ?? 0;
                $update->save();
            }
        }
        // if ($request->id_pakan) {
        //     $update = KartuStok::where('id', $request->id_pakan)->update(['jenis' => $request->pakan]);
        // }
        if ($request->key == 'ovk') {
            ...
            $stokan = StokKandang::where('kandang_id', $kandang_id->kandang)
                ->where('tipe', '1')
                ->where('produk_id', $request->jenis)->first()->sisa;
            if (!empty(DB::table('kartustok')->where('tipekartu', 'ovk')->where('hari', $request->hari_datang)->where('tipekartu', 'ovk')->first())) {
                // dd('cek');
                ...
                StokKandang::where('kandang_id', $kandang_id->kandang)
                    ->where('tipe', '1')
                    ->where('produk_id', $request->jenis)->update(['sisa' => ($stokan + $request->jumlah_awal - $request->jumlah - $request->jumlah_keluar)]);
                $data['status']         =   200;
                return $data;
            } else {
                $update                  =   new KartuStok;
