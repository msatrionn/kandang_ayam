@php
    ...
    header("Content-Disposition: attachment; filename=Data Recording.xls");
@endphp

<table border="1">
    <thead>
        <tr>
            <th class="text-center border sticky-col sticky-head first-col" style="z-index: 98" rowspan="2">Tanggal</th>
        ...
        <tr>
            ...
            <th class="text-center sticky-heading sticky-col sticky-head minggu-col" style="z-index: 98">Minggu</th>
    ...
    <tbody>
        ...
        @for ($i = 1; $i <= 91; $i++)
            @php
                $mati   =   Populasi::view_record($kandang->id, $i)->populasi_mati ?? 0 ;
                ...
                $gram_ekor  =   Kartu::view_pakan($kandang->id, $i, 'keluar')  ? round(Kartu::view_pakan($kandang->id, $i, 'keluar') / ($kandang->populasi - $hidup), 2) : 0;
                $kumulatif  +=  $gram_ekor ;
            @endphp
            <tr>
                <td class="sticky-col first-col" style="z-index: 97">{{ Tanggal::date(Carbon\Carbon::parse($kandang->tanggal)->addDays(($i - 1))) }}</td>
                <td class="text-center sticky-col hari-col" style="z-index: 97">{{ $i }}</td>
                @if ($i%7 == 1)
                    <td class="text-center sticky-col minggu-col" style="z-index: 97" rowspan="7">{{ ($i%7 == 1) ? round($i / 7) + 1 : '' }}</td>
                @endif
                ...
                <td class="text-center table-secondary">{{ Populasi::view_record($kandang->id, $i) ? ($kandang->populasi - $hidup) : '' }}</td>
                <td class="text-center">{{ (Kartu::view_pakan($kandang->id, $i, 'masuk') > 0) ? Kartu::view_pakan($kandang->id, $i, 'masuk') : '' }}</td>
                <td class="text-center">{{ (Kartu::view_pakan($kandang->id, $i, 'keluar') > 0) ? Kartu::view_pakan($kandang->id, $i, 'keluar') : '' }}</td>
                <td class="text-center">{{ (Kartu::view_pakan($kandang->id, $i, 'masuk') + Kartu::view_pakan($kandang->id, $i, 'keluar')) ? ($masuk - $keluar) : '' }}</td>
                ...
                <td class="text-center">{{ (Timbang::hasil_timbang($kandang->id, $i) > 0) ? Timbang::hasil_timbang($kandang->id, $i) : '' }}</td>
                ...
                <td>
                    ...
                    <div class="small"> {{ Catatan::where('riwayat_id', $kandang->id)->where('hari', $i)->first()->user->name ?? '' }}</div>
                </td>
            </tr>
            @if ($i%7 == 0)
            @php
                $fcr    =   Timbang::timbang_mingguan($kandang->id, (7 * round($i / 7))) > 0 ? ($kumulatif / Timbang::timbang_mingguan($kandang->id, (7 * round($i / 7)))) : 0 ;
            @endphp
            <tr class="bg-light">
                <td class="font-weight-bold sticky-col first-col" style="z-index: 97" colspan="3">Sub Total</td>
                <td class="text-center font-weight-bold">{{ Populasi::sub_total($kandang->id, (7 * round($i / 7)), 'mati') }}</td>
                <td class="text-center font-weight-bold">{{ Populasi::sub_total($kandang->id, (7 * round($i / 7)), 'afkir') }}</td>
                <td class="text-center font-weight-bold">{{ Populasi::sub_total($kandang->id, (7 * round($i / 7)), 'panen') }}</td>
                ...
                <td class="text-center font-weight-bold">{{ Timbang::timbang_mingguan($kandang->id, (7 * round($i / 7))) }}</td>
                <td class="font-weight-bold" colspan="2">Unifomity : {{ Timbang::unifomity($kandang->id, (7 * round($i / 7))) }}</td>
                <td class="text-center font-weight-bold"></td>
            </tr>
            @endif
        @endfor
